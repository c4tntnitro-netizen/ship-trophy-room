...
public final class GanEdenTransitSystemGenerator {
    ...
    private static final int ACCESS_VERSION = 6;
    ...
    public static SectorEntityToken ensureGenerated() {
        ...
        if (gate != null) {
            ...
            if (GanEdenQuestManager.hasBothHypershuntLogs()) {
    ...
    private static void ensureTransverseOnlyAccess(StarSystemAPI system) {
        ...
        // Older layouts autogenerated a second nascent well whose target was
        // the system center. FractureJumpAbility uses the target of whichever
        // nearby well it selects, so leaving that entity in hyperspace could
        // still deposit the player directly on the Gate and parked Ordo even
        // after the authored arrival anchor was moved far away.
        if (removeCompetingNascentWells(system)) {
            rebuildAnchor = true;
        }
    ...
    /** Removes stale autogenerated wells that lead to this system's center. */
    private static boolean removeCompetingNascentWells(
            StarSystemAPI system) {
        boolean removed = false;
        for (SectorEntityToken entity : new ArrayList<SectorEntityToken>(
                Global.getSector().getHyperspace().getAllEntities())) {
            if (!(entity instanceof NascentGravityWellAPI)
                    || GRAVITY_WELL_ID.equals(entity.getId())) {
                continue;
            }
            NascentGravityWellAPI well = (NascentGravityWellAPI) entity;
            SectorEntityToken target = well.getTarget();
            if (target != null && target.getContainingLocation() == system) {
                Global.getSector().getHyperspace().removeEntity(well);
                removed = true;
            }
        }
        if (system.getAutogeneratedNascentWellsInHyper() != null
                && !system.getAutogeneratedNascentWellsInHyper().isEmpty()) {
