...
Gan Eden has no markets, objectives, ambient NPC fleets, jump points, nascent gravity wells, or hyperspace anchor. A silver-gold Sinistral and Dextral Shard stand guard near the Eden Transit Ring. Each defeated Shard fractures into two Facets and six three-craft Aspect wings; each defeated Facet fractures into two half-hull Tesseracts and six more Aspect wings. The golden Facets have 33% less hull than their ordinary counterparts, and the Tesseracts are terminal. Defeating the guardians yields an aureate cache calibrated to four times the Omega-weapon loot of the standard two-Tesseract encounter. The inert transit ring provides no interaction or return route. The system is tagged as cut off and hidden, excluded from random mission targeting, and cannot become a respawn location.
