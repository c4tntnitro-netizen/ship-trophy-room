...
Gan Eden is a deliberately compact, completely siloed star system built inside a prototype Dyson sphere. Its interior is presented as a fixed forced-perspective cutaway: curved inhabited geography fills a huge rounded triangular opening, closely framed by photovoltaic cells, structural ribs, and Domain-era machinery. The real campaign star is aligned with the cleared sun position in that painted environment. The visible aperture is also the actual playable boundary; an **Altitude Warning** field softly decelerates approaching fleets and rebounds them away from each side of the surrounding shell without causing hull, combat-readiness, or crew damage.

Gan Eden has no markets, objectives, ambient NPC fleets, jump points, nascent gravity wells, or hyperspace anchor. On the player's first arrival, a golden Sinistral and Dextral Shard intercept the fleet. Each defeated Shard fractures into two Facets and six three-craft Aspect wings; each defeated Facet fractures into two half-hull Tesseracts and six more Aspect wings. The golden Facets have 33% less hull than their ordinary counterparts, and the Tesseracts are terminal. An inert Eden Transit Ring marks the intended arrival point but provides no interaction or return route. The system is tagged as cut off and hidden, excluded from random mission targeting, and cannot become a respawn location.
